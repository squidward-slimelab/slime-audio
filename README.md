...
python3 scripts/slime_audio_live_edit.py add-effect --id break-loop-reverb-tail --type reverb --target break-loop --start 01:31.500 --duration 00:02.000 --tail-ms 3500 --wet 0.38 --gain-db -10 --delay-ms 80 --feedback 0.46 --room-size 0.72 --damping 0.55 --lowpass-hz 5200
...
DJ effects are planned session events in the top-level `effects` collection. Rendered primitives are `echo` and `reverb`, targetable to a clip, `deck:<name>`, `master`, or `all` with `start`, `duration`, `tail_ms`, `wet`, `gain_db`, `delay_ms`, `feedback`, and optional `lowpass_hz`. Reverb also reads `room_size` and `damping`; keep it as a wet send with modest `wet`, negative `gain_db`, and a low-pass so tails lift a double or brake-style moment without washing out the master. Mixdown renders only the wet effect copy, pads silence for the configured tail, diffuses reverb with dense early reflections plus all-pass smearing instead of a few discrete repeats, fades the end of every configured tail so finite windows do not chop abruptly, includes tails in routine audition windows, and the dashboard shows effects and their tails on a separate effects lane. Use `add-effect` for custom planned effects, `echo-stabs` for a doubled deck with echo, and `echo-drop` for a doubled deck with a reverb tail. Recipes that still need unavailable slip/brake behavior continue to refuse instead of degrading silently.
