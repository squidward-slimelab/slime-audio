...
Mix planning turns the analysis into executable session edits. `slime_audio_mix_planner.py` reads the live runner state as a lock, analyzes only current/future clips, and only creates overlays when the transition clears tempo/key compatibility gates. Safe overlays get phrase-sized starts, explicit click-protection fade-ins, deck filter/EQ carving, rendered tempo/key correction within configured limits, and top-level `transition_plans` that record the decision for each adjacent pair. Incoming drop-double previews are opt-in via `--double-every`; do not add them by default. Use `--cached-analysis-only --horizon-ms 1200000` for live-safe planning that never decodes new audio on the critical path and only rewrites a bounded future block. Use `--max-render-pitch-shift-semitones 0` when a routine should preserve original keys instead of allowing key correction. Unsafe or missing-analysis transitions remain explicit cuts; the renderer does not add implicit auto-crossfades just because clips overlap. Run the planner immediately after importing a playlist and again when extending the future set; a straight import is not a finished DJ set.
...
When the native session runner starts real playback, it writes `runtime/active-set.json` with the session and state paths it is using. The dashboard follows that pointer by default before falling back to `runtime/mix-session*`. This keeps named live sessions visible without manually restarting the web UI with matching `--state` and `--session` arguments. Use runner `--no-active-pointer` only for isolated diagnostics where the dashboard should not move.
...
python3 scripts/slime_audio_live_edit.py instant-double-routine --source-id break-loop --id break-loop-reverb --recipe echo-drop --start 01:24.000 --cache runtime/dj-analysis-cache.json
...
python3 scripts/slime_audio_live_edit.py instant-double-routine --source-id break-loop --id break-loop-slip-brake --recipe slip-brake --start 01:24.000 --cache runtime/dj-analysis-cache.json
python3 scripts/slime_audio_live_edit.py instant-double-routine --source-id break-loop --id break-loop-brake --recipe brake-drop --start 01:24.000 --cache runtime/dj-analysis-cache.json
...
python3 scripts/slime_audio_live_edit.py add-effect --id break-loop-echo-tail --type echo --target break-loop --start 01:31.500 --duration 00:02.000 --tail-ms 3000 --wet 0.4 --feedback 0.45 --lowpass-hz 4200
python3 scripts/slime_audio_live_edit.py add-effect --id break-loop-reverb-tail --type reverb --preset medium-room --target break-loop --start 01:31.500 --duration 00:02.000
python3 scripts/slime_audio_live_edit.py add-effect --id break-loop-brake --type vinyl_brake --target break-loop --start 01:31.500 --duration 00:00.500
...
`instant-double-routine` is the named recipe layer for same-track duplicate moves. Raw `instant-double` remains available for hand-built moves; it clones an active or future source clip onto a free deck at the same musical position, preserving path, trim position, rendered tempo/pitch, and gain. Use `--cue-kind` on routines when the move should start from a stored hook/drop/build cue rather than a hand-entered timestamp. Use `--gate-beats` for simple quantized routines, `--gate-offset-beats 1/2` for first-cut-on-the-AND timing, and `--cut-source` when the duplicate should trade against the original instead of stacking on top of it; the dashboard labels the duplicate as an instant double instead of a normal song transition. `scratch-cuts` creates attached reverse/rate-shifted scratch clips on the source deck's effect lane and locally ducks the parent clip so the scratch replaces the source while the source timeline keeps moving underneath. Keep scratch clips sparse and continuous, roughly 140-260 ms, instead of micro-slicing them into glitch stutters.

Slip/flux events live in `slip_events`. They mark a temporary manipulated target clip while the source clip continues underneath, with stored `source_start_ms` and `source_resume_ms` so render windows can prove the source playhead returns where it would have been. Use raw `slip` for hand-built scratch/brake gestures, or `slip-brake` when the brake should be a temporary color that returns on phrase. Use `brake-drop` when the brake should change timing: it splits the source, inserts a one-beat vinyl brake as an attached effect track, then resumes the source late from the pre-brake source position.
...
Per-track levels have separate static trim and deck mixer moves. Clip `trim_db` is input trim for matching source loudness before performance moves. Clip `gain_db` is static clip placement, but performance fader/EQ/filter moves belong in top-level `deck_automations` targeted to a deck such as `deck-2`; mixdown applies those absolute-timeline points to whichever clip is on that deck. Use `scripts/slime_audio_live_edit.py automate --target deck-2 --param gain_db ...` for channel fader moves, and target `deck-2` with `eq_low_db`, `eq_mid_db`, `eq_high_db`, `lowpass_hz`, or `highpass_hz` for knob moves. Clip-targeted `gain_db` automation remains supported for clip-local replacement gates such as scratches, but it is not the normal mixer fader representation. `eq_low_db` applies a low shelf around 120 Hz, `eq_mid_db` applies a 1 kHz bell, and `eq_high_db` applies a high shelf around 6.5 kHz. Use it to carve beds under a lead before reaching for blunt low-pass/high-pass filters; for example, reduce a techno bed's lows and upper bite while leaving enough mid motion to hear the rhythm.

DJ effects are planned session events in the top-level `effects` collection. Rendered primitives are `echo`, `reverb`, and `vinyl_brake`, targetable to a clip, `deck:<name>`, `master`, or `all` with `start`, `duration`, `tail_ms`, `wet`, `gain_db`, `delay_ms`, `feedback`, and optional `lowpass_hz`. Echo renders as clean delayed wet taps that decay by `feedback`; it should sound like repeats, not like a comb-filter wobble. Reverb also reads `room_size` and `damping`; its command defaults are mapped from Audacity's Reverb defaults: room size 75%, pre-delay 10 ms, reverberance/damping 50%, wet/dry gain -1 dB, and full stereo width. Use `--preset` for Audacity factory starting points: `defaults`, `acoustic`, `ambience`, `artificial`, `clean`, `modern`, `vocal-i`, `vocal-ii`, `dance-vocal`, `modern-vocal`, `voice-tail`, `bathroom`, `small-room-bright`, `small-room-dark`, `medium-room`, `large-room`, `church-hall`, `cathedral`, and `big-cave`; explicit controls such as `--wet`, `--tail-ms`, or `--room-size` override the preset. Mixdown renders only the wet effect copy, pads silence for the configured tail, and renders reverb with the LADSPA `zita-reverb` plugin from the tiny `rev-plugins` package. Vinyl brake renders a deterministic speed/pitch slowdown to silence; use short beat-derived durations. Clips can also use `reverse` plus `playback_rate` for scratch-style record motion where pitch and speed move together. Routine-generated brake/scratch artifacts are `effect-track` clips attached to their source deck, so the dashboard shows them under the deck as child lanes instead of consuming normal music lanes. Use `add-effect` for custom planned effects, `echo-stabs` for a doubled deck with echo, `echo-drop` for a doubled deck with a reverb tail, `scratch-cuts` for slip/transform scratches that replace the source through fader cuts, `slip-brake` for a phrase-safe brake color, and `brake-drop` for a timing-changing brake.
...
Lean-ins are scheduled session events, not immediate side streams. A lean-in has an exact mix timeline `start`, spoken text, voice `volume`, and paired `duck_volume`/`lowpass_hz` automation. `add-mic` defaults lean-ins to `deck-5`, the dedicated vocal lane; keep that lane routed `THRU` so crossfader or bed utility automation cannot hide vocal balance. `scripts/slime_audio_session_mixdown.py` renders those events into one Snapcast-ready audio file so voice, ducking, and low-pass filtering happen in the shared mix instead of relying on the old receiver packet path.
