...
Crossfader routing is the controller-style cut layer. Store deck assignments in `fader_routing.deck_assignments`, normally `deck-1`/`deck-3` on side `A` and `deck-2`/`deck-4` on side `B`, then automate `crossfader.position` from `-1` (hard A) through `0` (center, both sides) to `1` (hard B). Mixdown translates crossfader motion into deterministic per-clip deck gains, and the dashboard shows the fader lane separately from normal gain automation. Doubled-deck routines with `--cut-source` use crossfader cuts instead of unrelated source/duplicate gain envelopes.
