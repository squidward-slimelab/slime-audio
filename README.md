...
Slip/flux events live in `slip_events`. They mark a temporary manipulated target clip while the source clip continues underneath, with stored `source_start_ms` and `source_resume_ms` so render windows can prove the source playhead returns where it would have been. Use raw `slip` for hand-built scratch/brake gestures, or `brake-drop` to clone an instant double, route every non-brake deck to the source side, cut the crossfader to the duplicate for one beat, and apply a vinyl brake while the source keeps advancing silently behind the move.
