...
DJ effects are planned session events in the top-level `effects` collection. The first rendered primitive is `echo`, targetable to a clip or deck with `start`, `duration`, `tail_ms`, `wet`, `gain_db`, `delay_ms`, `feedback`, and optional `lowpass_hz`. Mixdown renders echo as a deterministic wet delayed stream with a tail, routine auditions include effect tails in their window, and the dashboard shows effects on a separate effects lane. Use `add-effect` for custom planned effects; use `echo-stabs` when a named instant-double routine should attach an echo tail to the doubled deck. Recipes that still need unavailable slip/brake/reverb behavior continue to refuse instead of degrading silently.
