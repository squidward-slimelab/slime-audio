...
- `continue` builds a first playable buffer (about 5 minutes of actual scheduled timeline by default, `--min-runway-ms`) from database candidates, runs the planner and structural beds, then launches the native runner in windowed mode. Only render validity and harmonic clashes on real overlaps can block the launch; the taste guards (vanilla coverage, bed balance, transition bookkeeping, stem-load conventions) run as advisories reported in the plan output for the DJ agent to weigh. `validate-session` still runs every check strictly for explicit QA. `continue` refuses to stomp healthy playback unless `--force`. `--target-bpm` sets the session's `master_bpm` (Ableton-style): clips carry their analyzed `source_bpm` and the session layer warps them to the master — straight or double/half-time — on every load, so blends are tempo-free and key-fit-only and live `master_bpm` edits retempo all future windows. Clips opt out individually with `warp: false` (`live_edit set-warp`); mic lean-ins are not clips and never warp. Selection pulls tracks reachable straight or at an octave.
- `extend` appends a fresh planned block (default `--block-ms` 5 minutes) to the live session behind the playhead. It resolves the live session from `runtime/active-set.json` (or `--session`/`--state`), no-ops when at least `--ahead-ms` of music remains ahead of the playhead or when `--target-length-ms` is reached (default 30 minutes; `0` extends forever), excludes tracks already in the session, plans the new block on a working copy with the same fatal/advisory split, and only publishes atomically if the live file did not change in the meantime. New event ids are namespaced with an `ext-` prefix so repeated extensions never collide.
