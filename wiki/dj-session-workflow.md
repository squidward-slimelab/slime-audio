...
Do not rely on implicit auto-crossfades. The renderer never invents fades; crossfader movement, overlap blends, and long fades must be explicit performance actions, planner transition plans, or named routines with a musical reason. Autodj runs the mix planner with `--allow-blends` by default (disable with `--no-planner-blends`), so compatible-key/tempo pairs get planned overlaps with transition carving while incompatible pairs stay explicit hard cuts. Protective click fades are fine when authored intentionally, but hidden default handoff fades are not.
