...
- Render the canonical DJ session timeline, including normal decks, attached effect lanes, the dedicated `deck-5` vocal lane, fader assignments, automation, effects, slip events, and mic lean-ins. Effects and slips render as chips inside the track lane of their target clip or deck (resolved client-side from the effect's target id); only unresolvable targets fall back to the fx utility lane, and the crossfader lane keeps an animated A/B handle.
- Order deck lanes as `deck-3`, `deck-1`, `MIC`, `deck-2`, `deck-4`, matching the physical/operator view. Each deck lane runs horizontally with a channel-strip header on the left: deck number, A/B/THRU routing badge, five hardware-style knobs (gain, low, mid, hi, filter) whose pointer rotation and colored rings animate with the playhead, and a level meter. The knob ring hues match the automation curve hues in the lane so a moving knob visually pairs with its curve.
...
- Update knobs, meters, and the crossfader handle from the animated playhead, not only from `/api/state` polling: one requestAnimationFrame loop writes CSS custom properties on persistent DOM (no per-frame innerHTML), knobs interpolate smoothly during active deck automation and glow while a moving segment is under the playhead, and every rack panel re-renders only when its own data signature changes. This is the anti-glitch architecture; keep hot paths free of DOM rebuilds.
