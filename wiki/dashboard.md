...
- Overlay per-track automation curves on timeline rows. Clip-owned automation is remapped from the generic automation events back onto the target clip lane, while crossfader automation remains on the fader lane. Hovering a curve shows automation values that are active at the hovered timestamp, rather than every future automation event on that deck.
