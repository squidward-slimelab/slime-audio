...
- `--target-key "A minor"` — set the session's master key: every keyed lead pitch-matches to it, with minor keys converting to their relative major before the pitch-step math (Am and C major are the same center). Material further than `--max-key-shift-semitones` (default 2) plays native. Keymatching is on per deck by default; free a track for manual pitch work with `live_edit set-warp --id X --no-keymatch` (its current pitch freezes as authored). Change live with `live_edit set-key --key "..."`; an empty key releases everything to native pitch. Under master tempo + master key together, nearly every handoff is blendable.
