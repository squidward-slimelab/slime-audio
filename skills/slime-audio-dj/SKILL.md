...
- **All live-edit timestamps are absolute session milliseconds**, never clip-relative — `automate`, `add-effect`, and friends reject (or worse, misplace) clip-relative offsets. Compute from the `junctions` list or the clip's `start_ms`.
