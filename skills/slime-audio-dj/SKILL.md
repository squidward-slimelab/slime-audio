...
0. **If the output says UNDERBUILT** (composed length short of your declared bound), the sizing `extend --track ...` is your VERY NEXT command — pick records to fill the printed gap in one shot. Do not research, verify, or edit first; three cold sets lost their front halves to exactly that.
...
#    set unless the operator asks. If your first pass composes SHORT of
#    the declared bound, `extend --track ...` will fill toward it (the
#    runway check steps aside while a declared bound is unmet); size the
#    addition to the remaining gap.
...
- **All live-edit timestamps are absolute session milliseconds**, never clip-relative — `automate`, `add-effect`, and friends reject (or worse, misplace) clip-relative offsets. Compute from the `junctions` list or the clip's `start_ms`. Stem-level automation targets take the `stem-group:<id>:<stem>` form.
