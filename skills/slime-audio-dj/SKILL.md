...
8. Add, move, trim, overlap, or automate clips by timestamp through the active live-edit wrapper. It defaults to `runtime/mix-session.json`, reads `runtime/mix-session-state.json` as the playhead lock, and records `live_edit_applied` history events. This is the normal way to change a set while it is playing; editing a copied JSON file and restarting playback should be reserved for setup or recovery.
   ...
   Slip/flux gestures live in `slip_events`, not hidden timeline edits. A slip event marks a temporary manipulated target clip while the source clip continues underneath, and stores `source_start_ms` plus `source_resume_ms` so the deck can return to the source position it would have reached naturally. Treat slip mode as a short performance flourish: schedule the move on a downbeat or clear phrase point, release it on a downbeat, and leave room for the track to breathe between moves. Useful slip-mode families are vinyl brake/pause, backspin, baby scratch, and hot-cue trigger gestures; do not scatter random scratch slices over a dry track. Use `scratch-cuts` for DJ-style transform scratches: it schedules short attached `scratch_motion` clips and hard crossfader throws so the scratch replaces the source while the source timeline keeps moving underneath. Scratch motion is tiny forward/back platter movement around one cue, not a long reversed phrase. Use `slip-brake` when the brake is only a phrase-safe color and must return exactly on time. Use `brake-drop` when the brake should actually change timing: it splits the source, inserts a one-beat attached brake effect track, and resumes the source late from the pre-brake source position. Use raw `slip` only when hand-building a custom manipulated clip.

   Track levels have separate static trim and performance gain. Use clip `trim_db` for input trim/loudness matching, then use clip `gain_db` and `gain_db` automation as the channel fader lane. DJ effects live in the session `effects` collection, not in hidden renderer flags. The available rendered primitives are `echo`, `reverb`, and `vinyl_brake`, with `start`, `duration`, `tail_ms`, `wet`, `gain_db`, `delay_ms`, `feedback`, and optional `lowpass_hz`; reverb also accepts `room_size` and `damping`. Echo should render as clean delayed wet taps that decay by `feedback`; avoid treating comb-filter wobble as a DJ echo throw. Reverb command defaults are mapped from Audacity's Reverb defaults: room size 75%, pre-delay 10 ms, reverberance/damping 50%, wet/dry gain -1 dB, and full stereo width. Use `--preset` for Audacity factory starting points: `defaults`, `acoustic`, `ambience`, `artificial`, `clean`, `modern`, `vocal-i`, `vocal-ii`, `dance-vocal`, `modern-vocal`, `voice-tail`, `bathroom`, `small-room-bright`, `small-room-dark`, `medium-room`, `large-room`, `church-hall`, `cathedral`, and `big-cave`. Presets are not taste decisions: pick one as a starting color, then override `--wet`, `--tail-ms`, `--gain-db`, `--room-size`, `--feedback`, or `--damping` for the moment. Effects can target a clip, `deck:<name>`, `master`, or `all`; prefer clip/deck sends unless the whole master really needs space. Mixdown renders only the wet copy, pads the configured tail, and renders reverb with the LADSPA `zita-reverb` plugin from the tiny `rev-plugins` package. Vinyl brake renders a deterministic speed/pitch slowdown to silence; keep it short and beat-derived. Clip `reverse` plus `playback_rate` renders record-motion playback where speed and pitch move together; `scratch_motion` renders micro forward/back platter slices around a cue so it does not sound like a reversed section. Routine-generated scratch/brake artifacts are `effect-track` clips attached to the source deck through `attached_deck` and `effect_parent_clip_id`, so the dashboard shows them as child lanes such as `deck-2-fx` instead of normal music lanes. Use `echo-stabs` when a named double should carry an echo tail, `echo-drop` when it should carry a reverb tail, `scratch-cuts` for source-replacing transform scratches, `slip-brake` for phrase-safe brake color, `brake-drop` for a real timing brake, and raw `add-effect` for custom planned moves.
