...
A set is **records woven together over a persistent groove** — the body of the set is remixed, not just the junctions. Under a master tempo, generation weaves by default: each chapter's opening record donates its drums as a foundation groove running under the following leads, the next record's vocal teases over the current one's instrumental tail, and the planner authors double-deck junctions (drums-first entries, midpoint bass swaps) wherever split material exists. Full songs still travel through the set (`--arrangement full`), but a playlist with transitions — however good the transitions — is not the assignment. Chopping tracks into anchored sections is a specific technique for rapid stem-remix work (`--arrangement sections`, implied by `--remix-focus`), not how normal sets are built.
